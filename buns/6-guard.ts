...
const stepAdd = (a: Coords, b: Coords): Coords => [a[0]+b[0], a[1]+b[1]];

const takeStep = (pos: [number, number], facing: Facing, obstructions: Set<string>, width: number, height: number):
    [Coords, Facing] | false => {
    const step = FACING_DELTAS[facing];
    const newPos: Coords = stepAdd(pos, step);
...
console.log('Test: Expected 41 Actual', runPath(testInput.split('\n')));

const part1In = await Bun.file('6.txt').text().then(f => f.trim().split('\n'));
console.log('Part 1', runPath(part1In));